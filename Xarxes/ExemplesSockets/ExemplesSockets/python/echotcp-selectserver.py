        ...
        while True:
            ...
            for x in i:
                if x is sock:
                    ...
                    if newdata:
                        # Noves dades
                        print(f'{len(newdata)} from {adrs[x]}')
                        data[x]=data.get(x,'')+newdata.decode()
                        if x not in outsocks:
                            outsocks.append(x)
                    else:
                        # Desconnexio
                        print(f'desconn: {adrs[x]}')
                        del adrs[x]
                        try:
                            outsocks.remove(x)
                        except ValueError:
            ...
            for x in o:
                aenviar=data.get(x)
                if aenviar:
                    nsnt = x.send(aenviar.encode())
                    print(f'{nsnt} to -> {adrs[x]}')
...
if __name__ == '__main__':
    try:
        ...
        if options.verbose: print(f'{time.asctime()}')

        main()

        now_time = time.time()
        if options.verbose: print(f'{time.asctime()}')
        if options.verbose: print(f'TOTAL TIME: {(now_time - start_time)} (seconds)')
        if options.verbose: print(f'          : {datetime.timedelta(seconds=(now_time - start_time))}')
        sys.exit(0)
